...
Depends: libc6 (>= 2.28), libgcc-s1, libgssapi-krb5-2, libstdc++6, zlib1g, libx11-6, libice6, libsm6, libfontconfig1, libssl3 | libssl3t64, libicu70 | libicu72 | libicu74 | libicu76 | libicu78, xdg-desktop-portal
Recommends: ffmpeg, tesseract-ocr, tesseract-ocr-eng
Description: FrameForge cross-platform desktop preview
 Local screen capture, annotation, scrolling image stitching, video recording,
 and OCR. This preview is not yet feature-equivalent to the Windows WPF edition.
"""
    desktop = """[Desktop Entry]
Type=Application
Name=FrameForge Desktop Preview
...
"""
    entries = [(p, "opt/frameforge-desktop/" + p.relative_to(stage).as_posix(),
                0o755 if p.name == "FrameForge.Desktop" or p.suffix == ".so" else 0o644)
...
def package(rid, dotnet, skip_build):
    ...
    if rid.startswith("osx"):
        ...
        info = {"CFBundleName": "FrameForge", "CFBundleDisplayName": "FrameForge Desktop Preview",
    ...
    entries = [(p, prefix + "/" + p.relative_to(source).as_posix(),
                0o755 if p.name == "FrameForge.Desktop" or p.suffix in (".so", ".dylib") else 0o644)
...
if __name__ == "__main__":
    ...
    package(options.rid, options.dotnet, options.skip_build)
